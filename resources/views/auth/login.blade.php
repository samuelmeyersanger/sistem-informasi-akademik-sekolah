<x-guest-layout>
    <div class="min-h-[85vh] flex flex-col justify-center items-center px-4 w-full max-w-md mx-auto relative z-10">
        ...
        {{-- Kotak Login Utama: Glassmorphism Mewah --}}
        <div class="w-full bg-white/80 backdrop-blur-2xl p-8 md:p-10 rounded-[2.5rem] border border-white/60 shadow-2xl shadow-indigo-500/10 transition-all duration-300">
            ...
            <div class="mb-8 text-center">
                ...
                <div class="flex items-center justify-center gap-5 mb-5 relative">
                    {{-- Pendaran Cahaya (Glow) Halus di Balik Logo --}}
                ...
                <p class="text-xs text-slate-500 mt-3 max-w-xs mx-auto leading-relaxed font-medium">
                    Silakan masuk (login) untuk mengakses fitur e-Rapor, Peminjaman Sarpras, dan Data Akademik Terpadu.
                </p>
            </div>

            <x-auth-session-status class="mb-5 p-3.5 rounded-2xl text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 shadow-sm text-center" :status="session('status')" />
                ...
                <div class="flex items-center justify-between pt-1">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer select-none group">
                        <input id="remember_me" 
                               type="checkbox" 
                               name="remember"
                               class="w-4 h-4 rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500 focus:ring-offset-1 cursor-pointer transition-colors" />
                        <span class="ms-2 text-[11px] font-bold text-slate-500 group-hover:text-slate-800 transition-colors uppercase tracking-wider mt-0.5">
                            Ingat Sesi Saya
                        </span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-[11px] font-bold text-indigo-600 hover:text-indigo-700 transition-all uppercase tracking-wider hover:underline decoration-indigo-300 underline-offset-4" 
            ...
            <div class="mt-8 pt-5 border-t border-slate-100 text-center">
                <a href="/" class="text-[11px] font-bold text-slate-400 hover:text-indigo-600 transition-colors inline-flex items-center gap-1.5 uppercase tracking-wider group">
                    <i class="fa-solid fa-house group-hover:-translate-y-0.5 transition-transform"></i> Kembali ke Beranda
                </a>
            </div>
