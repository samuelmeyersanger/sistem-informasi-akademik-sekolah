<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800 leading-tight flex items-center gap-3">
            <span class="text-3xl">🏫</span> {{ __('Ruang Kelas & Penugasan Wali') }}
        </h2>
    </x-slot>
    ...
    <div x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-3xl border border-gray-100">
                ...
                <div class="p-6 border-b border-gray-100 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
                    <div>
                        <h3 class="text-lg font-black text-gray-900 mb-1">Daftar Ruang Kelas</h3>
                        <p class="text-sm text-gray-500">Kelola master ruang kelas, tingkat belajar, penugasan wali kelas definitif, dan lihat jadwal KBM.</p>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-4">
                        <form action="{{ route('kesiswaan.kelas.index') }}" method="GET" class="flex items-center gap-2 w-full sm:w-auto relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400 text-lg">🔍</span>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari ruang kelas atau wali..." class="text-sm rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 shadow-inner w-full sm:w-72 py-3 pl-12 pr-12 transition-colors">
                            
                            @if(request('search'))
                                <a href="{{ route('kesiswaan.kelas.index') }}" class="absolute inset-y-0 right-16 flex items-center pr-2 text-gray-400 hover:text-rose-500 font-bold text-lg transition-colors cursor-pointer" title="Hapus Pencarian">
                                    &times;
                                </a>
                            @endif
                        ...
                        @if(auth()->user()->hasPermission('kesiswaan.kelas.store'))
                            <button @click="openCreate = true" class="px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg transition-transform transform hover:-translate-y-0.5 flex items-center justify-center gap-2 shrink-0">
                                <span class="text-lg">➕</span> Kelas Baru
                            </button>
                        @endif
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-gray-50/80 border-b border-gray-100 text-gray-500 font-bold uppercase tracking-wider text-xs">
                                ...
                                <th class="p-5 w-64">Nama Ruangan</th>
                                <th class="p-5 w-80">Wali Kelas Definitif</th>
                                <th class="p-5 pr-8 text-center w-96">Navigasi & Aksi</th>
                            </tr>
                        </thead>
                            ...
                            @forelse($kelas as $item)
                                <tr class="hover:bg-indigo-50/30 transition-colors group">
                                    ...
                                    <td class="p-5 align-middle">
                                        <div class="font-black text-gray-900 text-lg mb-1 flex items-center gap-2">
                                            <span>🏫</span> Kelas {{ $item->nama_kelas }}
                                        </div>
                                    </td>
                                    <td class="p-5 align-middle">
                                        @if($item->waliKelas)
                                    ...
                                    <td class="p-5 pr-8 text-center align-middle">
                                        <div class="flex items-center justify-center gap-2 flex-wrap">
                                            ...
                                            <a href="{{ route('kesiswaan.kelas.show', $item->id) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-teal-50 hover:bg-teal-100 border border-teal-100 text-teal-700 font-bold rounded-xl text-xs transition-colors shadow-sm" title="Anggota Siswa Kelas">
                                                <span>🔍</span> Anggota Siswa
                                            </a>
                                            
                                            <a href="{{ route('kesiswaan.kelas.jadwal', $item->id) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-100 text-blue-700 font-bold rounded-xl text-xs transition-colors shadow-sm" title="Manajemen Jadwal KBM">
                                                <span>📅</span> Jadwal KBM
                                            </a>

                                            <div class="h-6 w-px bg-gray-200 mx-1"></div>

                                            <button type="button" @click="initEdit('{{ route('kesiswaan.kelas.update', $item->id) }}', '{{ $item->tingkat }}', '{{ $item->nama_kelas }}', '{{ $item->wali_kelas_id }}')" class="p-2.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 text-indigo-600 font-bold rounded-xl text-xs cursor-pointer transition-colors shadow-sm" title="Edit Data">
                                                ✏️
                                            </button>
                                            
                                            @if(auth()->user()->hasPermission('kesiswaan.kelas.destroy'))
                                                <button type="button" @click="initDelete('{{ route('kesiswaan.kelas.destroy', $item->id) }}', 'Kelas {{ addslashes($item->nama_kelas) }}')" class="p-2.5 bg-rose-50 hover:bg-rose-100 border border-rose-100 text-rose-600 font-bold rounded-xl text-xs cursor-pointer transition-colors shadow-sm" title="Hapus Data">
                                                    🗑️
                                                </button>
                                            @endif
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="p-16 text-center text-gray-400 bg-gray-50/30">
                                        ...
                                        @else
                                            <span class="text-5xl block mb-4">📭</span>
                                            <p class="text-lg font-bold text-gray-500">Sistem belum memiliki referensi ruang kelas.</p>
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden" @click.away="openCreate = false">
                ...
                <div class="flex justify-between items-center border-b border-gray-100 p-6 bg-gray-50">
                    <div>
                        <h3 class="text-lg font-black text-gray-900">➕ Registrasi Ruang Kelas</h3>
                ...
                <form action="{{ route('kesiswaan.kelas.store') }}" method="POST">
                    @csrf
                    <div class="p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Identitas Kelas <span class="text-rose-500">*</span></label>
                                <input type="text" name="nama_kelas" required placeholder="Misal: 7-A, VIII B" class="w-full text-sm font-semibold rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm bg-gray-50 px-4 py-3 uppercase">
                            </div>
                        </div>

                        <div class="p-5 bg-blue-50/50 border border-blue-100 rounded-2xl">
                            <label class="block text-sm font-bold text-blue-900 mb-2">Wali Kelas Pendamping (Opsional)</label>
                    ...
                    <div class="px-8 py-5 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition-colors cursor-pointer">💾 Simpan Kelas</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden" @click.away="openEdit = false">
                ...
                <div class="flex justify-between items-center border-b border-gray-100 p-6 bg-gray-50">
                    <div>
                        <h3 class="text-lg font-black text-gray-900">✏️ Modifikasi Ruang Kelas</h3>
                        <p class="text-xs text-indigo-600 font-bold mt-1 inline-flex items-center gap-1"><span class="w-1.5 h-1.5 bg-indigo-500 rounded-full"></span> Sinkronisasi Hak Akses Presensi Otomatis</p>
                    </div>
                ...
                <form :action="editActionUrl" method="POST">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Identitas Kelas <span class="text-rose-500">*</span></label>
                                <input type="text" name="nama_kelas" x-model="editNamaKelas" required class="w-full text-sm font-semibold rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm bg-gray-50 px-4 py-3 uppercase">
                            </div>
                        </div>

                        <div class="p-5 bg-blue-50/50 border border-blue-100 rounded-2xl">
                            <label class="block text-sm font-bold text-blue-900 mb-2">Wali Kelas Definitif</label>
                    ...
                    <div class="px-8 py-5 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition-colors cursor-pointer">💾 Terapkan Ubahan</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl p-8 text-center relative overflow-hidden" @click.away="openDelete = false">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mb-5 px-2">
                        Anda akan menghapus data <strong class="text-gray-800" x-text="deleteTargetName"></strong>.
                    </p>
                    <div class="p-4 bg-rose-50/50 border border-rose-100 rounded-xl mb-8">
                        <p class="text-xs font-bold text-rose-700 leading-relaxed text-left">
                            🚨 <strong>Peringatan Penghapusan Berseri (Cascade):</strong><br>Seluruh histori nilai, presensi, jadwal KBM, dan pemetaan keanggotaan siswa aktif yang terkait dengan kelas ini akan dilenyapkan.
                        </p>
                    </div>
                </div>
