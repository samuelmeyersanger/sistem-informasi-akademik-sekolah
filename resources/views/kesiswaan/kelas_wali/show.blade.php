<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Detail Kelompok Wali & Anggota Siswa') }}
            </h2>
            <a href="{{ route('kesiswaan.kelas_wali.index') }}" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-lg transition-colors flex items-center gap-1">
                ⬅️ Kembali ke Daftar Kelompok
            </a>
        </div>
    </x-slot>
    ...
    <div x-data="{
        openPlotting: false,
        openMutasiMassal: false, // Digunakan untuk form Naik/Tinggal Kelas
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Kelompok {{ $kelas->nama_kelas }}</h3>
                ...
                <form action="{{ route('kesiswaan.kelas_wali.show', $kelas->id) }}" method="GET" class="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-100">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                ...
                <div class="p-6 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-gray-50/50">
                    <div>
                        <h4 class="text-base font-bold text-gray-900">Daftar Anggota Kelompok</h4>
                        <p class="text-xs text-gray-500">Jumlah total peserta didik aktif pada kelompok wali ini: <strong>{{ count($anggota) }} Siswa</strong>.</p>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <a href="{{ route('kesiswaan.kelas_wali.anggota.downloadTemplate', ['id' => $kelas->id, 'semester_id' => $current_semester_id]) }}" class="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-black text-xs font-semibold rounded-lg shadow-sm transition-all flex items-center gap-1">
                            📥 Download Template
                        </a>
                                ...
                                @forelse($anggota as $item)
                                    <tr class="hover:bg-gray-50/80 transition-colors">
                                        ...
                                        <td class="p-4 pr-6 text-center">
                                            <button type="button" @click="triggerConfirm('{{ route('kesiswaan.kelas_wali.anggota.remove', $item->id) }}', '{{ $item->siswa->nama_lengkap }}')" class="p-1 text-rose-600 hover:text-rose-800 hover:underline font-medium cursor-pointer">
                                                ❌ Keluarkan
                                            </button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="p-12 text-center text-gray-400 italic bg-gray-50/30">
                                            Belum ada siswa yang di-plotting masuk ke dalam kelompok ini pada semester terpilih.
                                        </td>
                                    </tr>
                                @endforelse 
                            </tbody>
        ...
        <div x-show="openPlotting" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openPlotting = false">
                <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div>
                        <h3 class="text-sm font-bold text-gray-900 uppercase">Plotting Siswa Masuk Kelompok</h3>
                        <p class="text-[10px] text-gray-400 mt-0.5">Hanya menampilkan siswa aktif yang belum memiliki kelompok wali di semester terpilih.</p>
                    </div>
                ...
                <form action="{{ route('kesiswaan.kelas_wali.anggota.store') }}" method="POST" class="space-y-3 text-left">
                    @csrf
                    <!-- Sesuai dengan request validasi controller (kelas_wali_id) -->
                    ...
                    <div>
                        ...
                        <select name="siswa_ids[]" multiple required class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm h-48">
                            ...
                            @empty
                                <option disabled>-- Tidak ada siswa kosong/tanpa kelompok wali saat ini --</option>
                            @endforelse
                        </select>
        ...
        <div x-show="openImport" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openImport = false">
                <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div>
                        <h3 class="text-sm font-bold text-gray-900 uppercase">Import Anggota Kelompok Massal</h3>
                ...
                <form action="{{ route('kesiswaan.kelas_wali.anggota.import', $kelas->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 text-left">
                    @csrf
                    <input type="hidden" name="semester_id" value="{{ $current_semester_id }}">
                    ...
                    <div class="p-3 bg-blue-50/70 border border-blue-100 rounded-xl text-[11px] text-blue-800 leading-relaxed space-y-1">
                        ...
                        <ol class="list-decimal list-inside space-y-0.5 pl-0.5">
                            ...
                            <li>Ganti huruf <strong class="text-rose-600">N</strong> menjadi <strong class="text-emerald-600">Y</strong> pada kolom paling kanan untuk siswa yang ingin dimasukkan ke kelompok ini.</li>
        ...
        <div x-show="openMutasiMassal" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openMutasiMassal = false">
                <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div>
                        <h3 class="text-sm font-bold text-gray-900 uppercase">Proses Kenaikan / Tinggal Kelas</h3>
                        <p class="text-[10px] text-amber-600 font-medium mt-0.5">Memproses pemindahan siswa yang dicentang.</p>
                    </div>
                ...
                <form action="{{ route('kesiswaan.kelas_wali.anggota.mutasi') }}" method="POST" class="space-y-4 text-left">
                    @csrf
                    <!-- Sesuai dengan request controller -->
                    ...
                    <div>
                        ...
                        <select name="status_aksi" required class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            <option value="Naik Kelas">Naik Kelas (Pindah tingkat & kelompok baru)</option>
                            <option value="Tinggal Kelas">Tinggal Kelas (Tetap di tingkat yang sama)</option>
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Target Kelompok Baru *</label>
                            <select name="ke_kelas_id" required class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                                @foreach($all_kelas as $k)
                                    <option value="{{ $k->id }}">{{ $k->nama_kelas }} (Grade {{ $k->tingkat }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-black text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer">Eksekusi Proses</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        @if($kelas->tingkat == 9)
            <div x-show="openKelulusanMassal" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
                <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openKelulusanMassal = false">
                    ...
                    <form action="{{ route('kesiswaan.kelas_wali.anggota.kelulusan') }}" method="POST" class="space-y-3 text-left">
                        @csrf
                        <input type="hidden" name="dari_kelas_id" value="{{ $kelas->id }}">
                        <input type="hidden" name="semester_id" value="{{ $current_semester_id }}">
                        ...
                        <div>
                            ...
                            <input type="number" name="tahun_lulus" required value="{{ date('Y') }}" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                        </div>
        ...
        <div x-show="openConfirmDelete" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-sm w-full shadow-2xl border border-gray-100 p-6 text-center space-y-4" @click.away="openConfirmDelete = false">
                ...
                <div>
                    <h3 class="text-base font-bold text-gray-900">Keluarkan Anggota Kelompok?</h3>
                    <p class="text-xs text-gray-500 mt-2 leading-relaxed">
                        Apakah Anda yakin ingin mengeluarkan <span class="font-bold text-gray-800" x-text="siswaNama"></span> dari kelompok ini?
                    </p>
                </div>
