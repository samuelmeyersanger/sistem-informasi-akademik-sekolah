    ...
    <div x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                ...
                <div class="p-6 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-gray-50/50">
                    <div>
                        <h3 class="text-base font-bold text-gray-900">Daftar Kelompok Wali & Pembimbing</h3>
                        <p class="text-xs text-gray-500">Kelola master data kelompok pembimbingan, penentuan tingkat, dan penugasan Guru Wali.</p>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
                        <form action="{{ route('kesiswaan.kelas_wali.index') }}" method="GET" class="flex items-center gap-2 w-full sm:w-auto">
                            <div class="relative flex items-center w-full sm:w-auto">
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kelompok..." class="text-xs rounded-lg border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm w-full sm:w-56 pr-8">
                                
                                @if(request('search'))
                                    <a href="{{ route('kesiswaan.kelas_wali.index') }}" class="absolute right-2.5 text-gray-400 hover:text-gray-600 font-bold text-sm" title="Clear Search">
                                        &times;
                                    </a>
                                @endif
                            </div>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-gray-100/70 border-b border-gray-100 text-gray-600 font-semibold text-xs uppercase tracking-wider">
                                ...
                                <th class="p-4">Wali / Pembimbing</th>
                                <th class="p-4 pr-6 text-center w-64">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-gray-700 text-xs">
                            @forelse($kelasWali as $item)
                                <tr class="hover:bg-gray-50/80 transition-colors">
                                    ...
                                    <td class="p-4">
                                        <div class="font-bold text-gray-900 text-sm">🏫 {{ $item->nama_kelas }}</div>
                                    </td>
                                    <td class="p-4">
                                        @if($item->waliKelas)
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="p-12 text-center text-gray-400 italic bg-gray-50/30">
                                        ...
                                        @else
                                            Belum ada master data kelompok wali yang terdaftar.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openCreate = false">
                <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div>
                        <h3 class="text-sm font-bold text-gray-900 uppercase">Tambah Kelompok Wali Baru</h3>
                        <p class="text-[10px] text-emerald-600 font-medium mt-0.5">ℹ️ Kelompok akan dikunci otomatis pada periode Semester Aktif.</p>
                    </div>
                ...
                <form action="{{ route('kesiswaan.kelas_wali.store') }}" method="POST" class="space-y-3 text-left">
                    @csrf
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Nama Kelompok Wali *</label>
                        <input type="text" name="nama_kelas" required placeholder="Contoh: Asrama A, Halaqoh B" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Pilih Wali Pembimbing (Opsional)</label>
                        <select name="wali_kelas_id" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            <option value="">-- Tanpa Wali Dahulu --</option>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex justify-end gap-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer">Simpan Kelompok</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openEdit = false">
                <div class="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div>
                        <h3 class="text-sm font-bold text-gray-900 uppercase">Ubah Detail Kelompok</h3>
                        <p class="text-[10px] text-indigo-600 font-medium mt-0.5">ℹ️ Perubahan plotting wali akan mempengaruhi hak akses pembimbing.</p>
                    </div>
                ...
                <form :action="editActionUrl" method="POST" class="space-y-3 text-left">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Wali Pembimbing</label>
        ...
        <div x-show="openDelete" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" style="display: none;" x-transition>
            <div class="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-sm w-full p-6 text-center space-y-4" @click.away="openDelete = false">
                ...
                <div>
                    <h4 class="text-sm font-bold text-gray-900">Hapus Master Kelompok Wali?</h4>
                    ...
                    <p class="text-[10px] text-rose-600 mt-2 bg-rose-50/70 p-2.5 rounded-lg leading-relaxed border border-rose-100">
                        🚨 <strong>Peringatan Cascade Hapus:</strong> Seluruh data anggota yang tergabung dalam kelompok ini akan ikut terhapus dari basis data.
                    </p>
                </div>
