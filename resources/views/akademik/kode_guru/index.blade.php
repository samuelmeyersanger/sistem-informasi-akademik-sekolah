<x-app-layout>
    ...
    <div x-data="{
        ...
        initEdit(k) {
            ...
            this.editMapelId = k.mata_pelajaran_id ?? '';
            this.editJumlahJamMengajar = k.jumlah_jam_mengajar;
            this.openEdit = true;
        },
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100 text-gray-600 font-semibold text-xs uppercase tracking-wider">
                                ...
                                <th class="p-4 text-center w-32">Beban Jam</th>
                            ...
                            @forelse($kodeGuru as $k)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="p-4 pl-6 text-center">
                                        <span class="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full font-mono font-bold text-xs">
                                            {{ $k->kode }}
                                        </span>
                                    </td>
                                    <td class="p-4">
                                        <div class="font-bold text-gray-900 text-sm">
                                            {{ $k->pegawai->lengkap ?? 'Belum Ditentukan' }}
                                        </div>
                                    ...
                                    <td class="p-4">
                                        <div class="font-medium text-gray-800">
                                            {{ $k->mataPelajaran->nama_mapel ?? 'Mata Pelajaran Dihapus' }}
                                        </div>
                                        @if($k->mataPelajaran && $k->mataPelajaran->singkatan_mapel)
                                            <div class="text-[10px] text-indigo-500 font-bold uppercase">{{ $k->mataPelajaran->singkatan_mapel }}</div>
                                        @endif
                                    </td>
                                    <td class="p-4 text-center font-bold text-gray-900 text-sm">
                                        {{ $k->jumlah_jam_mengajar }} JP
                                    </td>
                                    <td class="p-4 pr-6 text-center">
                                        <div class="flex items-center justify-center gap-4">
                                            ...
                                            <button type="button" @click="initDelete('{{ route('akademik.kode-guru.destroy', $k->id) }}', '{{ addslashes($k->kode) }}')" class="text-rose-600 hover:underline font-semibold cursor-pointer">
                                                🗑️ Hapus
                                            </button>
                                        </div>
                                    </td>
                ...
                <form action="{{ route('akademik.kode-guru.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="kode" required placeholder="Cth: G01" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm uppercase">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Jml Jam Mengajar *</label>
                            <input type="number" name="jumlah_jam_mengajar" required min="0" value="0" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Pilih Guru / Pegawai *</label>
                        <select name="pegawai_id" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Pilih Mata Pelajaran *</label>
                        <select name="mata_pelajaran_id" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            <option value="">-- Pilih Mapel --</option>
                            @foreach($daftarMapel as $m)
                                <option value="{{ $m->id }}">[{{ $m->singkatan_mapel }}] {{ $m->nama_mapel }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="bg-white rounded-2xl max-w-sm w-full shadow-2xl border border-gray-100 p-6 space-y-4" @click.away="openEdit = false">
                ...
                <form :action="editActionUrl" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Kode Guru *</label>
                            <input type="text" name="kode" x-model="editKode" required class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm uppercase">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Jml Jam Mengajar *</label>
                            <input type="number" name="jumlah_jam_mengajar" x-model="editJumlahJamMengajar" required min="0" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Pilih Guru / Pegawai *</label>
                        <select name="pegawai_id" x-model="editPegawaiId" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            <option value="">-- Pilih Pegawai --</option>
                            @foreach($daftarPegawai as $peg)
                                <option value="{{ $peg->id }}">{{ $peg->nama_lengkap }}</option>
                            @endforeach
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Pilih Mata Pelajaran *</label>
                        <select name="mata_pelajaran_id" x-model="editMapelId" class="w-full text-xs rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            <option value="">-- Pilih Mapel --</option>
                            @foreach($daftarMapel as $m)
                                <option value="{{ $m->id }}">[{{ $m->singkatan_mapel }}] {{ $m->nama_mapel }}</option>
                            @endforeach
